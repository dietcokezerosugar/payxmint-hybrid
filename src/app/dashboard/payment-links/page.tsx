    ...
    <div className="space-y-6 md:space-y-8 pb-24 font-sans px-2 md:px-0 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between px-2 md:px-0">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-xl shadow-lg transition-all active:scale-95 text-[11px] font-black uppercase tracking-widest ${
      ...
      <div className="px-2 md:px-0">
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-white border border-slate-200 py-4 pl-14 pr-6 rounded-xl text-sm font-bold text-slate-900 focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 outline-none transition-all shadow-sm placeholder:text-slate-300"
          />
        </div>
      </div>
      ...
      <AnimatePresence>
        {showCreate && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 10 }}
            className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 space-y-8 shadow-xl relative z-20"
          >
            ...
            <div className="grid gap-6 md:grid-cols-3">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 outline-none transition-all placeholder:text-slate-300"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  type="number"
                  placeholder="99.00"
                  className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-black focus:bg-white focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 outline-none transition-all placeholder:text-slate-300"
                />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="flex gap-4 bg-slate-50 -mx-6 -mb-6 md:-mx-8 md:-mb-8 p-6 md:p-8 border-t border-slate-100">
              <button
                ...
                className="w-full py-4 bg-blue-600 text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all disabled:opacity-50 shadow-lg shadow-blue-600/20 active:scale-95"
              >
        ...
        <AnimatePresence mode="popLayout">
          {filteredLinks.length === 0 ? (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white rounded-2xl border-2 border-dashed border-slate-100 p-16 text-center"
            >
            ...
            filteredLinks.map((link) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5">
                  ...
                  <div className="text-right shrink-0 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div className="flex items-center gap-2 border-t md:border-0 pt-4 md:pt-0">
                  <button onClick={() => copyUrl(link.slug)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-white text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200 hover:bg-slate-50 active:scale-95 shadow-sm">
                    <Copy className="w-3.5 h-3.5" />
                    <span>Copy</span>
                  </button>
                  <a href={`/link/${link.slug}`} target="_blank" className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md shadow-blue-600/20 active:scale-95">
                    <ExternalLink className="w-3.5 h-3.5" />
                    <span>View</span>
                  </a>
                  <button onClick={() => deleteLink(link.id)} className="p-3 text-rose-500 bg-rose-50 border border-rose-100 rounded-xl hover:bg-rose-100 transition-colors active:scale-95" title="Delete">
